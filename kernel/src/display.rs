...
pub fn draw_bgra_image(fb: &Framebuffer, data: *const u8, size: usize) -> bool {
    ...
    blit_bgra_cover(fb, pixels, w, h);
    true
}

pub fn draw_bgra_image_fit_rect(
    fb: &Framebuffer,
    data: *const u8,
    size: usize,
    dst_x: usize,
    ...
    if fb.base.is_null() || dst_w == 0 || dst_h == 0 || size < 8 {
        return false;
    }
    let src_w = read_u32_le(data) as usize;
    let src_h = read_u32_le(unsafe { data.add(4) }) as usize;
    if src_w == 0 || src_h == 0 {
        return false;
    }
    ...
    let fit_width = (src_w as u64) * (dst_h as u64) > (dst_w as u64) * (src_h as u64);
    ...
    let out_x = dst_x + dst_w.saturating_sub(draw_w) / 2;
    let out_y = dst_y + dst_h.saturating_sub(draw_h) / 2;
    let step_x = ((src_w as u32) << 16) / (draw_w as u32);
    let step_y = ((src_h as u32) << 16) / (draw_h as u32);
    let mut sy_fp = 0u32;
    for y in 0..draw_h {
        let sy = (sy_fp >> 16) as usize;
        let row = unsafe { pixels.add(sy.min(src_h - 1) * src_w * 4) };
        let mut sx_fp = 0u32;
        for x in 0..draw_w {
            let sx = (sx_fp >> 16) as usize;
            let px = unsafe { row.add(sx.min(src_w - 1) * 4) };
            let b = unsafe { px.read() } as u32;
            let g = unsafe { px.add(1).read() } as u32;
            let r = unsafe { px.add(2).read() } as u32;
            put_pixel(fb, out_x + x, out_y + y, (r << 16) | (g << 8) | b);
...
fn blit_bgra_cover(fb: &Framebuffer, src: *const u8, src_w: usize, src_h: usize) {
    let dst_w = fb.width;
    let dst_h = fb.height;
    ...
    let src_ratio = (src_w as u64) * (dst_h as u64);
    let dst_ratio = (dst_w as u64) * (src_h as u64);
    ...
    if src_ratio > dst_ratio {
        sample_w = ceil_div((src_h as u64) * (dst_w as u64), dst_h as u64) as usize;
        if sample_w == 0 {
            sample_w = 1;
        }
        if sample_w < src_w {
        ...
        sample_h = ceil_div((src_w as u64) * (dst_h as u64), dst_w as u64) as usize;
        if sample_h == 0 {
            sample_h = 1;
        }
        if sample_h < src_h {
    ...
    let step_x = ((sample_w as u32) << 16) / (dst_w as u32);
    let step_y = ((sample_h as u32) << 16) / (dst_h as u32);
    let mut sy_fp: u32 = 0;
