...
impl Notepad {
    pub fn new(_fb: Framebuffer, fs_img: Option<ModuleRange>) -> Self {
        ...
        let base = b"HouseOffice Writer";
        title[..base.len()].copy_from_slice(base);
        let mut status = [0u8; 64];
        let status_text = b"Ready";
        status[..status_text.len()].copy_from_slice(status_text);
        Self {
            visible: false,
            win_x: 0,
            win_y: 0,
    ...
    pub fn open_empty(&mut self, fb: &Framebuffer) {
        self.show(fb);
        self.set_title(b"OFFICE.TXT");
        self.text_len = 0;
        self.scroll = 0;
        self.set_status(b"New document. Save writes OFFICE.TXT");
    }
    ...
    pub fn open_file(
        ...
        if let Some(fs) = fs_img.and_then(Fat32::new) {
            ...
            self.set_status(b"Loaded. Save writes OFFICE.TXT");
        } else {
    ...
    pub fn handle_click(&mut self, fb: &Framebuffer, x: usize, y: usize) -> bool {
        ...
        let save_rect = (wx + PAD, body_y + 4, 72, 22);
        if hit(x, y, save_rect.0, save_rect.1, save_rect.2, save_rect.3) {
            self.save_office_file();
            self.redraw(fb);
            return true;
        }

        let scroll_x = wx + ww.saturating_sub(PAD + SCROLL_W);
        let scroll_y = body_y + PAD + TOOLBAR_H;
        let scroll_h = body_h.saturating_sub(PAD * 2 + TOOLBAR_H + STATUS_H);
    ...
    pub fn redraw(&mut self, fb: &Framebuffer) {
        ...
        fill_vertical_gradient(
            ...
            chrome.content_w,
            TOOLBAR_H,
            if is_dark { 0x002B3038 } else { 0x00F8FBFF },
            if is_dark { 0x0021262E } else { 0x00EDF3FC },
        );
        display::fill_rect(
            fb,
            chrome.content_x,
            chrome.content_y + TOOLBAR_H.saturating_sub(1),
            chrome.content_w,
            1,
            if is_dark { 0x00424B58 } else { 0x00D6E2F0 },
        );
        let save_rect = (chrome.content_x + PAD, chrome.content_y + 4, 72, 22);
        draw_button(fb, &mut writer, save_rect, b"Save", is_dark, text_color);
        writer.set_color(detail);
        writer.set_pos(save_rect.0 + save_rect.2 + 12, chrome.content_y + 9);
        writer.write_bytes(b"HOUSE_OS\\FILES\\OFFICE.TXT");

        let text_x = chrome.content_x + PAD;
        let text_y = chrome.content_y + PAD + TOOLBAR_H;
        let text_w = chrome.content_w.saturating_sub(PAD * 2 + SCROLL_W + 4);
        let text_h = chrome.content_h.saturating_sub(PAD * 2 + TOOLBAR_H + STATUS_H);
        ...
        let scroll_y = text_y;
        let scroll_h = chrome.content_h.saturating_sub(PAD * 2 + TOOLBAR_H + STATUS_H);
        ...
        let status_y = chrome.content_y + chrome.content_h.saturating_sub(STATUS_H);
        display::fill_rect(
            fb,
            chrome.content_x,
            status_y,
            chrome.content_w,
            1,
            if is_dark { 0x00343B46 } else { 0x00D6E2F0 },
        );
        writer.set_color(detail);
        writer.set_pos(chrome.content_x + PAD, status_y + 4);
    ...
    fn save_office_file(&mut self) -> bool {
        if let Some(range) = self.fs_img {
            let ok = unsafe {
                crate::fat32::overwrite_existing_file(
                    range,
                    &[&b"HOUSE_OS"[..], &b"FILES"[..]],
                    b"OFFICE.TXT",
                    &self.text[..self.text_len],
            ...
            if ok {
                self.set_status(b"Saved to HOUSE_OS\\FILES\\OFFICE.TXT");
            } else {
                self.set_status(b"Save failed: OFFICE.TXT not writable");
            }
            ok
        } else {
            self.set_status(b"Save failed: no filesystem image");
            false
        }
    }

    fn set_title(&mut self, file_name: &[u8]) {
        let prefix = b"HouseOffice - ";
        self.title_len = 0;
        for &b in prefix {
        ...
        if self.title_len == prefix.len() {
            self.title[0..11].copy_from_slice(b"HouseOffice");
            self.title_len = 11;
        }
    }
    ...
    fn layout_metrics(&self, fb: &Framebuffer) -> ((usize, usize), (usize, usize)) {
        ...
        let text_h = content_h.saturating_sub(PAD * 2 + TOOLBAR_H + STATUS_H);
        let max_cols = (text_w / 8).max(1);
        let max_lines = (text_h / LINE_HEIGHT).max(1);
...
fn sanitize_loaded_text(buf: &mut [u8], len: usize) -> usize {
    ...
    while out > 0 && (buf[out - 1] == b' ' || buf[out - 1] == 0) {
        out -= 1;
    }
    out
}
...
fn draw_button(
    ...
    display::fill_rect(
        ...
        rect.2,
        1,
        if is_dark { 0x00616B78 } else { 0x00FFFFFF },
    );
    display::fill_rect(
        fb,
        rect.0,
        rect.1 + rect.3.saturating_sub(1),
        rect.2,
        1,
        if is_dark { 0x00272D36 } else { 0x00B8C7D8 },
    );
    writer.set_color(text_color);
    let text_w = label.len() * 8;
    writer.set_pos(rect.0 + rect.2.saturating_sub(text_w) / 2, rect.1 + 7);
