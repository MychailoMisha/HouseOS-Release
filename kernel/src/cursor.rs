...
fn draw_raw(fb: &Framebuffer, x0: usize, y0: usize, data: *const u8, size: usize) -> bool {
    ...
    for row in 0..h {
        ...
        for col in 0..w {
            ...
            let rgb = (r << 16) | (g << 8) | b;
            if !is_transparent_cursor_pixel(r, g, b, rgb) {
                display::put_pixel(fb, px, py, rgb);
            }
        }
    }
...
fn is_transparent_cursor_pixel(r: u32, g: u32, b: u32, rgb: u32) -> bool {
    rgb == CURSOR_TRANSPARENT
        || (r > 140 && b > 140 && g < 170 && r > g.saturating_add(35) && b > g.saturating_add(35))
}
