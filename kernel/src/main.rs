...
const MB2_CHECKSUM: u32 = 0u32.wrapping_sub(MB2_HEADER_MAGIC.wrapping_add(MB2_ARCH).wrapping_add(MB2_HEADER_LEN));
    ...
    while tag_ptr + core::mem::size_of::<Mb2Tag>() <= info_end {
        ...
        match tag.typ {
            8 => {
                ...
                if fb_tag.fb_type == 1 && (fb_tag.size as usize) >= core::mem::size_of::<Mb2FramebufferRgbTag>() {
...
fn kernel_main(
    ...
    // ============================================================
    // [TTF] Завантаження шрифту з initrd
    // ============================================================
    vga_write(b"Bitmap text mode\n");
    ...
    let _disk = drivers::storage::ata_pio::init();
...
fn module_is_fat32(start: usize, end: usize) -> bool {
    ...
    for i in 0..4 {
        let entry = 446 + i * 16;
        let part_type = header[entry + 4];
        let is_fat32_part = part_type == 0x0B || part_type == 0x0C || part_type == 0x1B || part_type == 0x1C;
        if !is_fat32_part {
            continue;
        }
        let lba_start = read_u32_le_buf(header, entry + 8) as usize;
        if lba_start == 0 {
            continue;
        }
        ...
        let part_header = unsafe { core::slice::from_raw_parts((start + offset) as *const u8, 512) };
        if is_fat32_header(part_header) {
            return true;
        }
    }
    false
...
fn is_fat32_header(header: &[u8]) -> bool {
...
fn read_u32_le_buf(buf: &[u8], offset: usize) -> u32 {
    if offset + 4 > buf.len() {
        return 0;
    }
...
fn read_u32_le(ptr: *const u8) -> u32 {
    unsafe {
        (ptr.read() as u32) | ((ptr.add(1).read() as u32) << 8) | ((ptr.add(2).read() as u32) << 16) | ((ptr.add(3).read() as u32) << 24)
    }
}
...
fn find_tar_file(start: *const u8, end: *const u8, name: &[u8]) -> Option<(*const u8, usize)> {
    ...
    while (ptr as usize) + 512 <= end as usize {
        ...
        if file_name == name
            || (file_name.len() == name.len() + 2 && file_name[0] == b'.' && file_name[1] == b'/' && &file_name[2..] == name)
        {
...
impl TextWriter {
    ...
    pub fn write_bytes_sized(&mut self, bytes: &[u8], size: u16) {
        if size <= 10 {
            self.write_bytes(bytes);
            return;
        }
        let scale = (size as usize / 8).max(1).min(4);
        for &b in bytes {
            if b == b'\n' {
                self.new_line();
            } else {
                self.write_char_scaled(b, scale);
            }
        }
    }
    ...
    fn write_char_scaled(&mut self, ch: u8, scale: usize) {
        let glyph = FONT8X8_BASIC[ch as usize];
        for (row, bits) in glyph.iter().enumerate() {
            for col in 0..8 {
                if (bits >> col) & 1 == 1 {
                    let px = self.x + col * scale;
                    let py = self.y + row * scale;
                    display::fill_rect(&self.fb, px, py, scale, scale, self.fg);
                }
            }
        }
        self.x += 8 * scale;
        if self.x + 8 * scale >= self.fb.width {
            self.new_line();
        }
    }
