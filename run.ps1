...
if ($Boot -eq "iso") {
    ...
    $useHostPhysicalDisks = [bool]$HostPhysicalDisks
    if (-not $NoHostPhysicalDisks -and -not $useHostPhysicalDisks) {
        if (Test-IsAdministrator) {
            $useHostPhysicalDisks = $true
            Write-Host "Full Disk enabled: host physical disks will be attached read-only."
        } else {
            Write-Warning "Full Disk needs Administrator PowerShell to attach Windows disks read-only. Run as Administrator or pass -HostPhysicalDisks."
        }
    }
    & $runIso `
        ...
        -HostPhysicalDisks:$useHostPhysicalDisks `
        -AllowHostDiskWrite:$AllowHostDiskWrite `
        -HostDiskIndexes $HostDiskIndexes `
        -ImagePath $ImagePath
    return
}
